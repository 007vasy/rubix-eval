...
const data = await fetch("/api/leaderboard/ai", { cache: "no-store" }).then((r) => r.json());
const ais = data.ais || [];
if (!ais.length) {
  empty.hidden = false;
} else {
...
rows.addEventListener("click", (event) => {
  ...
  const list = (row.solved || [])
    .map(
      (s) => `<div class="alg"><b>${s.puzzle}</b><span>HTM ${s.htm ?? "—"}</span><span>${s.step_count ?? "—"} steps</span><span>${s.click_count ?? "—"} clicks</span><span>${fmtTime(s.elapsed_sec)}</span><span>${fmtTokens(s.tokens_used)}</span>${replayLink(s.record_id)}</div>`,
    )
    .join("");
  detail.hidden = false;
  detail.innerHTML = `<h2>${row.ai}</h2>
    <p>Hardest: <b>${row.hardest.puzzle}</b> · ${row.solves} solved challenge${row.solves === 1 ? "" : "s"}.</p>
    <div class="algs">${list}</div>`;
});
